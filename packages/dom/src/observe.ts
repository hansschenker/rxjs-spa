...
export function textChanges(el: HTMLInputElement | HTMLTextAreaElement) {
...
export function checkedChanges(el: HTMLInputElement) {
...
export function attrChanges(el: Element, attr: string) {
  // Observe changes to attributes using MutationObserver
  return new Observable<string | null>((subscriber) => {
    const observer = new MutationObserver(() => {
      subscriber.next(el.getAttribute(attr));
    });
...
export function hasClass(el: Element, className: string) {
  return attrChanges(el, "class").pipe(
    map(() => el.classList.contains(className)),
  );
}

export function classChanges(el: Element, className: string) {
